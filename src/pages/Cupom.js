...
function sendOrder() {
    ...
    if (open === 'true') {
        ...
        if (resp === true) {
            ...
            const msg = `📝 Pedido: *${npedido}* %0A👤 Cliente: *${cliente}* %0A💳 Forma de Pagamento: *${pagamento}* %0A📍 Endereço para entrega: *${entrega}* %0A%0A🛍️ Produtos: %0A%0A${produtos}%0A💬 Observações: ${dados[0].obs}%0A%0ATotal dos Produtos: *${totalProducts('total')}* %0ATaxa da Entrega: *${txEntrega('toString')}* %0ADesconto: *${totalProducts('desconto')}* %0ATotal: *${totalProducts('totalPedido')}*`
